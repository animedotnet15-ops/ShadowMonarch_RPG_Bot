...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    rows = [
        ...
        [InlineKeyboardButton(text="🧙 My Characters", callback_data="menu:characters"),
         InlineKeyboardButton(text="🏅 Achievements", callback_data="menu:achievements")],
...
def achievements_keyboard(unlocked_ids: list, titles: dict) -> InlineKeyboardMarkup:
    rows = []
    for aid in unlocked_ids:
        title = titles.get(aid, {}).get("title", aid)
        rows.append([InlineKeyboardButton(text=f"🎖️ Equip: {title}", callback_data=f"equiptitle:{aid}")])
    rows.append([InlineKeyboardButton(text="🏠 Main Menu", callback_data="menu:main")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
