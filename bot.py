...
async def track_group_activity(chat, user) -> None:
    """Registers the group and logs one activity point, if this happened in a group chat."""
...
async def setting_cmd(message: Message):
    ...
    await message.answer("⚙️ <b>Bot Settings</b>\nCustomize the /start welcome experience.", reply_markup=kb.settings_main_keyboard(ai_on))
...
async def set_db_channel_cmd(message: Message, command: CommandObject):
    ...
    await message.answer(f"✅ DB backup channel set to <code>{arg}</code>. Backups run automatically every 6 hours.")
    ...
    while True:
        try:
            for row in await database.due_hourly_reminders():
                try:
                    await bot.send_message(row["user_id"], "⏰ <b>Your hourly mission is ready to claim!</b>")
                except Exception:
                    pass
            ...
            for row in await database.due_energy_reminders():
                try:
                    await bot.send_message(row["user_id"], f"⚡ <b>Your energy is full ({row['max_energy']}/{row['max_energy']})!</b> Time to hit some gates.")
                except Exception:
                    pass
