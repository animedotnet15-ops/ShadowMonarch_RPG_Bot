...
class Database:
    ...
    async def claim_daily_with_streak(self, user_id: int) -> int:
        ...
        async with self.connection() as db:
            await db.execute(
                "UPDATE players SET daily_claimed_at=?, last_daily_date=?, streak_days=? WHERE user_id=?",
    ...
    async def due_hourly_reminders(self) -> list[aiosqlite.Row]:
        ...
        async with self.connection() as db:
            return await (
                await db.execute(
                    "SELECT user_id, first_name FROM players WHERE hourly_claimed_at <= ? AND hourly_reminded=0 AND reminders_enabled=1",
                    (cutoff,),
                )
            ).fetchall()
    ...
    async def due_energy_reminders(self) -> list[aiosqlite.Row]:
        async with self.connection() as db:
            return await (
                await db.execute(
                    "SELECT user_id, first_name, energy, max_energy FROM players "
